...
`;
  ...
  function startQuickSeller(ctx) {
    ...
    ctx.scheduler.listen(document, "keyup", (event) => {
      if (event.code === ctx.settings.hotkey) stop();
    }, { capture: true });
...
`;
  ...
  function startEnhancer(ctx) {
    ...
    async function gameRequest(command, match, strip = []) {
      return ctx.game.request(command, match, { strip, timeout: 7e3 });
    }
    ...
    async function enhance() {
      ...
      try {
        ...
        message(error.message || "Ulepszanie nie powiodło się.", true);
      } finally {
        busy = false;
        delete windowElement.dataset.busy;
      }
    }
