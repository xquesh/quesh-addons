...
`);
  ...
  function relogTarget(hero, page2) {
    ...
    if (!domain) {
      try {
        domain = new URL(engine2.worldConfig?.getApiDomain?.()).hostname.match(/(?:^|\.)margonem\.(pl|com)$/)?.[1];
      } catch {
      }
    }
...
${timers.length ? timers.map((timer) => `${timer.name}: ${timer.text}${timer.state === "window" ? " · możliwy respawn" : ""}`).join("\n") : settings.showTimers && !available ? "Dane timerów niedostępne" : "Brak aktywnych timerów"}`;
    ...
    function relog(hero) {
      ...
      try {
        ...
        updateTimers();
        relogTimeout = scheduler2.timeout(() => unlockRelog("Zmiana postaci nie zakończyła się. Możesz spróbować ponownie."), 1e4);
      } catch (cause) {
        reloggingFrom = "";
        reloggingObserved = false;
        relogStartedAt = 0;
