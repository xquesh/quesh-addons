(() => {
  ...
  function createGame(page2, events2, scheduler2, host = window) {
    ...
    function settleRequests(packet) {
      ...
      for (const request2 of [...pending]) {
        ...
        if (!match) continue;
        pending.delete(request2);
        scheduler2.clearTimeout(request2.timeout);
        request2.releaseAbort();
        request2.resolve(match);
        if (request2.strip.length) parsed = stripPacket(parsed, request2.strip);
      }
      return parsed;
    }
    ...
    function destroy() {
      ...
      for (const request2 of pending) {
        scheduler2.clearTimeout(request2.timeout);
        request2.releaseAbort();
        request2.reject(new Error("Komunikacja gry została zatrzymana."));
      }
      pending.clear();
    ...
    function request(command, match, options = {}) {
      return new Promise((resolve, reject) => {
        if (stopped || page2.Engine?.allInit !== true || typeof page2._g !== "function") return reject(new Error("Gra nie jest jeszcze gotowa."));
        ...
        const abort = () => {
          ...
          scheduler2.clearTimeout(entry.timeout);
          entry.releaseAbort();
        ...
        entry.timeout = scheduler2.timeout(() => {
  ...
  function createPanel(settings, styles2, scheduler2, events2) {
    ...
    scheduler2.listen(button, "click", () => panel2.hidden ? open() : close());
    ...
    mountButton();
    const buttonHostObserver = scheduler2.observer(MutationObserver, mountButton);
  ...
  function createHud(state, ctx, runtime) {
    ...
    function brightNeutral(color2) {
      if (!color2 || color2.a === 0) {
        return false;
      }
      const brightness = (color2.r + color2.g + color2.b) / 3;
      const neutrality = Math.max(color2.r, color2.g, color2.b) - Math.min(color2.r, color2.g, color2.b);
    ...
    function structuralShape(style) {
      ...
      if (state.settings.uiAutoBorder) {
        [
          ...
          ([side, width, color2]) => {
            if ((parseFloat(width) || 0) >= 0.5 && brightNeutral(parseCssColor(color2))) {
              sides.push(side);
            }
          }
        );
  ...
  function createTestLoot(state, ctx, runtime) {
    ...
    function testLoot() {
      ...
      const heroId3 = ctx.game.page.Engine?.hero?.d?.id ?? ctx.game.page.Engine?.hero?.id ?? ctx.game.page.g?.hero?.id ?? 0;
  ...
  function createDetection(state, ctx, runtime) {
    ...
    function getLegendaryLootItems(data) {
      ...
      return Object.values(items).filter(
        (item) => {
          ...
          return ["l", "loot", "k", "c", "colossus"].includes(loc) && isLegendary2(item);
        }
      );
    }
    ...
    return { isLegendary: isLegendary2, getLegendaryLootItems, processGameData };
  }
...
`);
  ...
  var bonusDefaults = { bonusFont: "Arial", bonusSize: 9, bonusColor: "#ffffff", bonusBold: true, bonusItalic: false, bonusShadow: "soft", bonusShadowColor: "#000000", bonusShadowStrength: 3 };
  var SHADOW_TYPES = /* @__PURE__ */ new Set(["none", "soft", "outline", "glow"]);
  ...
  function bonusCss(style) {
    return `font:${style.italic} ${style.weight} ${style.size}px/${style.size + 2}px ${style.family}!important;color:${style.color}!important;text-shadow:${bonusShadow(style)}!important;`;
  }
  ...
  function createTooltipTools(tooltipConfig) {
    const ITEM_TYPES2 = ["Pierścienie", "Naszyjniki", "Hełmy", "Rękawice", "Zbroje", "Dystansowe", "Strzały", "Buty", "Jednoręczne", "Półtoraręczne", "Dwuręczne", "Orby magiczne", "Tarcze", "Pomocnicze"];
    function isUpgradeableItem(html) {
      const match = html.match(/Typ:\s*([^<\n]+)/i);
      return match ? ITEM_TYPES2.some((type) => match[1].trim().includes(type)) : false;
    }
    ...
    function itemById(itemId3) {
      try {
        if (!itemId3) return null;
        const items = window.Engine?.items;
        if (typeof items?.getItemById === "function") {
          const direct = items.getItemById(itemId3);
          if (direct) return direct;
        }
        return items?.items?.[itemId3] || null;
      } catch {
        return null;
      }
    }
    ...
    function lootHtml(itemId3, info, fullHtml) {
      const hid = currentHid(fullHtml);
      let item = itemById(itemId3);
    ...
    function groundBadges() {
      ...
      for (const item of drawables) {
        ...
        const wrapper = function(canvas, ...args) {
          ...
          canvas.shadowColor = textStyle.shadowColor;
          canvas.shadowBlur = ["soft", "glow"].includes(textStyle.shadow) ? textStyle.shadowStrength : 0;
          canvas.shadowOffsetX = 0;
          canvas.shadowOffsetY = textStyle.shadow === "soft" ? 1 : 0;
          if (textStyle.shadow === "outline") {
            canvas.strokeStyle = textStyle.shadowColor;
            canvas.lineWidth = 2;
  ...
  function heroTimers(data, heroId3, now, fadeout = 600) {
    ...
    return data.filter((timer) => timer && String(timer.heroData?.id) === String(heroId3) && (timer.type === 2 || timer.type === "hero" || timer.user === 1) && Number.isFinite(Number(timer.presp)) && Number(timer.presp) > 0 && now - Number(timer.presp) < expiry).map((timer) => {
  ...
  function referenceRelog(characterId, world, page2) {
    if (page2.Engine.allInit !== true) return false;
    if (page2.Engine.changePlayer?.id !== null && page2.Engine.changePlayer?.id !== void 0) return false;
    const currentId = page2.Engine.hero.d.id || page2.getCookie("mchar_id") || void 0;
    if (characterId == currentId) throw new Error("Jesteś już na tej postaci!");
    if (typeof page2.Engine.dialogue === "object") return false;
    const expires = /* @__PURE__ */ new Date();
    expires.setTime(expires.getTime() + 2592e6);
    page2.setCookie("mchar_id", `${characterId}`, expires, "/", `margonem.${page2.getMainDomain()}`, true);
    page2.location.replace(`https://${world}.margonem.${page2.getMainDomain()}`);
    return true;
  }
...
`;
  ...
  function startRelogger(ctx) {
    ...
    function position() {
      ...
      const anchor = dock === "top" ? document.querySelector(".interface-layer .positioner.top, .positioner.top") : document.querySelector(".bottom-panel-of-bottom-positioner");
      ...
      const right = gameRect?.width > 0 ? Math.min(page2.innerWidth, gameRect.right) : page2.innerWidth;
      ...
      bar.style.top = `${Math.max(0, Math.min(page2.innerHeight - height, dock === "top" ? vertical : vertical - height))}px`;
      const barLeft = parseFloat(bar.style.left);
      details.style.left = `${Math.max(-barLeft, Math.min(0, page2.innerWidth - barLeft - 264))}px`;
...
${timers.length ? timers.map((timer) => `${timer.name}: ${timer.text}${timer.state === "window" ? " · możliwy respawn" : ""}`).join("\n") : settings.showTimers && !available ? "Dane timerów niedostępne" : "Brak aktywnych timerów"}`;
    ...
    function render() {
      ...
      worldMenu.replaceChildren(...worlds.map((world) => {
        const option = document.createElement("button");
        option.type = "button";
        option.dataset.world = world;
        option.textContent = world;
        option.setAttribute("aria-pressed", String(world === currentWorld));
        return option;
      }));
    ...
    async function load() {
      ...
      try {
        ...
        if (!scheduler2.disposed) error = cause.name === "AbortError" ? "Przekroczono czas pobierania. Odśwież postacie w ustawieniach." : "Nie udało się pobrać postaci. Odśwież postacie w ustawieniach.";
      } finally {
        scheduler2.clearTimeout(timeout);
        request = null;
        loading = false;
    ...
    function relog(hero) {
      if (!hero) return;
      try {
        referenceRelog(hero.id, hero.world, page2);
        error = "";
      } catch (cause) {
        error = cause.message;
        updateTimers();
    ...
    scheduler2.listen(cards, "wheel", (event) => {
    ...
    scheduler2.listen(worldButton, "click", () => {
    ...
    scheduler2.listen(worldMenu, "click", (event) => {
      const option = event.target.closest("[data-world]");
      if (option) {
        ctx.changeSettings({ selectedWorld: option.dataset.world });
        worldButton.focus();
      }
    });
    scheduler2.listen(document, "pointerdown", (event) => {
      if (!worldMenu.contains(event.target) && !worldButton.contains(event.target)) closeWorldMenu();
    });
    scheduler2.listen(bar, "keydown", (event) => {
      if (event.key === "Escape" && !worldMenu.hidden) {
        closeWorldMenu();
        worldButton.focus();
        event.stopPropagation();
      }
    ...
    return {
      ...
      renderSettings(ctx) {
        ...
        section.innerHTML = `<h2>Przelogawka</h2><label class="mtk-enabled"><input type="checkbox" data-enabled> Dodatek aktywny</label>
                ...
                </div><p>Zmiana postaci działa tak jak w przesłanej Przelogawce: zapisuje identyfikator postaci w cookie gry i od razu otwiera jej świat. Nie korzysta z natywnego okna ani dodatkowego mechanizmu odzyskiwania.</p><p>Skróty wybierają postacie w kolejności na belce, na wybranym świecie. Nie działają podczas wpisywania tekstu w polach formularzy i czacie.</p>
        ...
        ctx.scheduler.listen(section.querySelector("[data-reset]"), "click", () => {
          ctx.changeSettings({ horizontal: 100 });
          section.querySelector('[data-setting="horizontal"]').value = 100;
          section.querySelector("[data-position-value]").textContent = "100%";
          section.querySelector("[data-status]").textContent = "Przywrócono pozycję po prawej stronie.";
        });
        ctx.container.append(section);
      }
    };
  ...
  // src/addons/quick-seller/style.js
  var quickSellerStyle = `
#qaddons-quick-seller{position:fixed;left:8px;bottom:68px;z-index:31000;width:32px;height:26px;border:1px solid #777;border-radius:0;background:#050505;color:#eee;font:700 10px Arial;cursor:pointer}
#qaddons-quick-seller:hover,#qaddons-quick-seller[data-running="true"]{border-color:#fff;box-shadow:0 0 8px rgba(255,255,255,.75);color:#fff}
#qaddons-quick-seller:disabled{opacity:.42;cursor:not-allowed;box-shadow:none}
`;

  // src/addons/quick-seller/runtime.js
  function editable(target) {
    return target?.closest?.('input,textarea,select,[contenteditable="true"]');
  }
  function startQuickSeller(ctx) {
    const page2 = ctx.game.page;
    const button = document.createElement("button");
    ...
    let heldHotkey = "";
    function available() {
      const shop = page2.Engine?.shop;
      return Boolean(page2.Engine?.allInit === true && shop && shop !== false && typeof shop.greatMerchant === "function" && shop.basket && typeof shop.basket.finalize === "function");
    }
    function refresh() {
    ...
    function loop() {
      ...
      try {
        page2.Engine.shop.greatMerchant(1);
        page2.Engine.shop.greatMerchant(2);
        page2.Engine.shop.greatMerchant(3);
        page2.Engine.shop.basket.finalize();
      } catch (error) {
    ...
    ctx.scheduler.listen(button, "pointerdown", (event) => {
      event.preventDefault();
    ...
    ctx.scheduler.listen(document, "keyup", (event) => {
      if (event.code === heldHotkey) stop();
    }, { capture: true });
    ctx.scheduler.listen(page2, "blur", stop);
    ...
    const tick = () => {
      refresh();
      ctx.scheduler.timeout(tick, 600);
    };
    refresh();
    tick();
  }

  // src/addons/quick-seller/index.js
  function createQuickSeller() {
    return {
      id: "quick-seller",
      name: "Sprzedawczyk",
      ...
      renderSettings(ctx) {
        ...
        ctx.scheduler.listen(hotkey, "keydown", (event) => {
          event.preventDefault();
          event.stopPropagation();
          if (event.code === "Escape") ctx.changeSettings({ hotkey: "" });
          else ctx.changeSettings({ hotkey: event.code });
          sync();
        });
        ctx.events.on("addonChanged", (event) => {
  ...
  function itemEligible(item, settings, excluded = /* @__PURE__ */ new Set()) {
...
`;
  ...
  // src/addons/enhancer/runtime.js
  var SLOT_LABELS = { all: "Dowolny przedmiot", armor: "Pancerz", jewelry: "Biżuteria", weapon: "Broń" };
  function allItems(page2) {
    try {
      return Object.values(page2.Engine?.items?.testMyItems?.() || {});
    } catch {
      return [];
    }
  }
  function itemName(item) {
    return parseStats(item)?.name || item?.name || `Przedmiot #${item?.id}`;
  }
  ...
  function itemPreview(item) {
    ...
    const source = [...document.querySelectorAll(`.item-id-${id}`)].find((node) => !node.closest("#qaddons-enhancer"));
    ...
    wrapper.className = "qe-item-preview";
  ...
  function settingsWithDefaults(settings) {
    settings.rarity = { ...DEFAULTS2.rarity, ...settings.rarity };
    settings.types = { ...DEFAULTS2.types, ...settings.types };
    settings.targets || (settings.targets = {});
    return settings;
  }
  function startEnhancer(ctx) {
    const page2 = ctx.game.page;
    const settings = settingsWithDefaults(ctx.settings);
    const toggle = document.createElement("button");
    toggle.id = "qaddons-enhancer-toggle";
    toggle.type = "button";
    toggle.textContent = "UL";
    toggle.title = "Ulepszarka";
    const windowElement = document.createElement("section");
    windowElement.id = "qaddons-enhancer";
    windowElement.hidden = settings.windowOpen !== true;
    ...
    function targetMap() {
      var _a, _b;
      return (_a = settings.targets)[_b = heroId(page2)] || (_a[_b] = {});
    }
    function target(slot) {
      const id = Number(targetMap()[slot]);
      return allItems(page2).find((item) => Number(item.id) === id) || null;
    }
    function modeSlots() {
      return settings.mode === "regular" ? ["all"] : settings.mode === "type" ? ["armor", "jewelry", "weapon"] : ["all", "armor", "jewelry", "weapon"];
    }
    function message(text, error = false) {
      if (!ctx.enabled || ctx.scheduler.disposed) return;
      status.textContent = text;
      status.style.color = error ? "#ff7979" : "#bbb";
      if (settings.messages && page2.message) page2.message(text, error);
    }
    function persistActive() {
      if (settings.rememberActive) ctx.changeSettings({ active });
    }
    function candidateItems(group = null) {
      const excluded = new Set(Object.values(targetMap()).map(Number));
      return allItems(page2).filter((item) => itemEligible(item, settings, excluded) && (!group || TYPE_GROUPS[group].includes(Number(item.cl))));
    }
    function highlight() {
    ...
    async function gameRequest(command, match, strip = []) {
      if (page2.Engine?.allInit !== true) throw new Error("Gra nie jest jeszcze gotowa.");
      return ctx.game.request(command, match, { strip, timeout: 7e3, signal: ctx.scheduler.signal });
    }
    ...
    async function updateUsage() {
      try {
        ...
        limitLabel.textContent = "Limit: brak danych";
      }
    }
    ...
    async function validateAndSet(slot, item) {
      ...
      try {
        ...
        if (ctx.enabled) {
          refresh();
          message(error.message || "Nie można ulepszyć tego przedmiotu.", true);
        }
      }
    }
    async function usageReached() {
      await updateUsage();
      return usage && Number(usage.count) >= Number(usage.limit);
    }
    ...
    async function enhance() {
      if (busy) return;
      busy = true;
      windowElement.dataset.busy = "true";
      try {
        scan();
        if (await usageReached()) return message("Osiągnięto dzienny limit ulepszania.", true);
        const plan = [];
        if (settings.mode !== "regular") {
          for (const slot of ["armor", "jewelry", "weapon"]) if (target(slot)) plan.push([slot, slot]);
        }
        ...
        for (const [slot, group] of plan) {
          ...
          while (candidates.length) {
            ...
            const result = await enhanceTarget(slot, batch);
            usedAnything = true;
        ...
        scan();
        message(usedAnything ? "Ulepszanie zakończone." : "Brak przedmiotów spełniających filtry.");
      } catch (error) {
        if (ctx.enabled) message(error.message || "Ulepszanie nie powiodło się.", true);
      } finally {
        busy = false;
        if (!ctx.scheduler.disposed) delete windowElement.dataset.busy;
      }
    }
    ...
    ctx.scheduler.listen(windowElement.querySelector("[data-active]"), "click", () => {
      active = !active;
      persistActive();
      refresh();
    });
    ctx.scheduler.listen(windowElement.querySelector("[data-mode]"), "change", (event) => {
      ctx.changeSettings({ mode: event.target.value });
      refresh();
    });
    ctx.scheduler.listen(windowElement.querySelector("[data-enhance]"), "click", enhance);
    ctx.scheduler.listen(windowElement.querySelector("[data-rescan]"), "click", () => {
      scan();
      message("Bufor został odświeżony.");
    });
    ctx.scheduler.listen(slots, "contextmenu", (event) => {
      const slot = event.target.closest("[data-slot]")?.dataset.slot;
      if (!slot) return;
      event.preventDefault();
      delete targetMap()[slot];
      ctx.storage.save();
      renderProgress(null);
      refresh();
    });
    ...
    function dropOn(slot, id) {
      const item = allItems(page2).find((entry) => Number(entry.id) === Number(id));
      if (!slot || !item) {
        clearDrag();
        return;
      }
    ...
    ctx.scheduler.listen(document, "dragstart", (event) => {
      ...
      if (!id) return;
      draggedItemId = id;
      slots.dataset.dragging = "true";
      try {
        event.dataTransfer?.setData("text/qaddons-item-id", String(id));
      } catch {
      }
    }, { capture: true });
    ctx.scheduler.listen(slots, "dragover", (event) => {
      if (draggedItemId) {
        event.preventDefault();
        markDropTarget(event);
      }
    });
    ctx.scheduler.listen(slots, "dragleave", (event) => {
      if (!slots.contains(event.relatedTarget)) slots.querySelectorAll(".qe-slot").forEach((node) => delete node.dataset.drop);
    });
    ctx.scheduler.listen(slots, "drop", (event) => {
      ...
      event.preventDefault();
      delete slots.dataset.dragging;
      dropOn(event.target.closest("[data-slot]"), id);
    });
    ctx.scheduler.listen(document, "dragend", clearDrag, { capture: true });
    ctx.scheduler.listen(document, "keydown", (event) => {
      if (event.code === settings.hotkey && !event.repeat && !event.target.closest?.('input,textarea,select,[contenteditable="true"]')) {
        event.preventDefault();
        enhance();
      }
    }, { capture: true });
    let drag = null;
    ctx.scheduler.listen(windowElement.querySelector("header"), "pointerdown", (event) => {
      if (event.target.closest("button")) return;
      const rect = windowElement.getBoundingClientRect();
      drag = { x: event.clientX - rect.left, y: event.clientY - rect.top };
      event.currentTarget.setPointerCapture?.(event.pointerId);
    });
    ctx.scheduler.listen(windowElement.querySelector("header"), "pointermove", (event) => {
      if (!drag) return;
      windowElement.style.left = `${Math.max(0, event.clientX - drag.x)}px`;
      windowElement.style.top = `${Math.max(0, event.clientY - drag.y)}px`;
      windowElement.style.bottom = "auto";
    });
    ...
    ctx.events.on("gamePacket", (packet) => {
      const data = Array.isArray(packet) ? packet.flat(Infinity) : [packet];
      const update = data.find((entry) => entry?.enhancement?.progressing || entry?.enhancement?.upgradable);
      if (update) renderProgress(update.enhancement.progressing || update.enhancement.upgradable);
    });
    ctx.scheduler.cleanup(() => {
      clearDrag();
      document.querySelectorAll(".qaddons-enhancer-buffer").forEach((node) => node.classList.remove("qaddons-enhancer-buffer"));
    ...
    const tick = () => {
      if (active && !busy && page2.Engine?.allInit === true) {
        const count = scan().length;
        const threshold = Math.max(1, Math.min(126, Number(settings.bufferSize) || 25));
        const usable = settings.mode === "regular" ? target("all") ? candidateItems().length : 0 : ["armor", "jewelry", "weapon"].reduce((sum, slot) => sum + (target(slot) ? candidateItems(slot).length : 0), 0) + (settings.mode === "hybrid" && target("all") ? candidateItems().length : 0);
        let free = Infinity;
        try {
          free = (page2.Engine?.bags || []).slice(0, 3).reduce((sum, bag) => sum + (bag ? Number(bag[0]) - Number(bag[1]) : 0), 0);
        } catch {
        }
  ...
  // src/addons/enhancer/index.js
  var TYPE_LABELS = {
    oneHand: "Broń jednoręczna",
    twoHand: "Broń dwuręczna",
    oneHalf: "Broń półtoraręczna",
    distance: "Broń dystansowa",
    help: "Pomocnicza",
    wand: "Różdżki",
    orb: "Orby",
    armor: "Zbroje",
  ...
  function createEnhancer() {
    return {
      ...
      renderSettings(ctx) {
        ctx.settings.rarity = { ...DEFAULTS2.rarity, ...ctx.settings.rarity };
        ctx.settings.types = { ...DEFAULTS2.types, ...ctx.settings.types };
        const section = document.createElement("section");
        section.className = "mtk-addon-settings";
        ...
        function sync() {
          ...
          section.querySelectorAll("[data-type]").forEach((input) => {
            input.checked = ctx.settings.types[input.dataset.type] !== false;
          });
          hotkey.value = ctx.settings.hotkey || "Brak";
        ...
        section.querySelectorAll("[data-rarity]").forEach((input) => ctx.scheduler.listen(input, "change", () => {
          ctx.changeSettings({ rarity: { ...ctx.settings.rarity, [input.dataset.rarity]: input.checked } });
          sync();
        }));
        section.querySelectorAll("[data-type]").forEach((input) => ctx.scheduler.listen(input, "change", () => {
          ctx.changeSettings({ types: { ...ctx.settings.types, [input.dataset.type]: input.checked } });
          sync();
        }));
        ctx.scheduler.listen(hotkey, "keydown", (event) => {
          event.preventDefault();
          event.stopPropagation();
          ctx.changeSettings({ hotkey: event.code === "Escape" ? "" : event.code });
          sync();
        });
        ctx.events.on("addonChanged", (event) => {
          if (event.id === ctx.id) sync();
  ...
  // src/addons/loot-chances/data.js
  var DEFAULTS3 = {
    hideSolo: false,
    position: "top-right",
    fontSize: 12,
  ...
  function catchingChance(item, members) {
    if (!members.length) return null;
    const professions = requiredProfessions(item);
    const eligible = professions.length ? members.filter((member) => professions.includes(String(member.prof || "").toLowerCase())) : [...members];
  ...
  function chanceCss(settings) {
    ...
    const shadows = {
      none: "none",
      soft: "0 1px 2px #000,0 0 3px #000",
      outline: "-1px -1px 0 #000,1px -1px 0 #000,-1px 1px 0 #000,1px 1px 0 #000,0 0 3px #000"
    };
    const colors = settings.customColors === false ? { none: "#ffffff", low: "#ffffff", mid: "#ffffff", high: "#ffffff" } : {
      none: safeColor(settings.colorNone, "#b30000"),
      low: safeColor(settings.colorLow, "#c25a00"),
      mid: safeColor(settings.colorMid, "#bd9700"),
      high: safeColor(settings.colorHigh, "#0ba800")
    };
...
`;
  ...
  // src/addons/loot-chances/runtime.js
  function packetList(packet) {
    return Array.isArray(packet) ? packet.flat(Infinity).filter(Boolean) : [packet];
  }
  function heroId2(page2) {
    return page2.Engine?.hero?.d?.id ?? page2.Engine?.hero?.id ?? page2.hero?.id ?? page2.g?.hero?.id;
  }
  function fightMembers(packet, page2) {
    ...
    for (const fighter of fighters) {
      ...
      if (!unique.has(id)) unique.set(id, {
        ...
        prof: String(fighter.prof).toLowerCase(),
        isHero: id === ownId
      });
    }
  ...
  function findIcon(root, entry, index) {
    ...
    const exact = icons.find((node) => node.dataset?.itemId === id || node.dataset?.id === id || [...node.classList].includes(`item-id-${id}`));
  ...
  function startLootChances(ctx) {
    ...
    ctx.events.on("lootClosed", () => {
      latest = null;
      clear();
    });
    ctx.events.on("lootChancesChanged", () => {
      applyStyles();
      render();
    });
  ...
  function calendarReminder(calendar, now = /* @__PURE__ */ new Date()) {
    if (!calendar?.start_ts) return null;
    const start2 = new Date(Number(calendar.start_ts) * 1e3);
    if (Number.isNaN(start2.getTime())) return null;
    const index = Math.round((warsawDateNumber(now) - warsawDateNumber(start2)) / 864e5);
    const day = calendar.days?.[index];
    return !day || day.isOpened ? null : { dayNo: index + 1 };
  }
  function freePromotions(promotions) {
  ...
  function expiredItems(page2) {
    let items;
    try {
      items = page2.Engine?.items?.testMyItems?.();
    } catch {
      return [];
    }
    return Object.values(items || {}).filter((item) => {
      try {
        return item?.id != null && Boolean(item?.checkExpires?.());
      } catch {
        return false;
      }
    });
...
`;
  ...
  // src/addons/reminder/runtime.js
  ...
  function startReminder(ctx) {
    ...
    const panel2 = document.createElement("section");
    panel2.id = "qaddons-reminder";
    panel2.hidden = true;
    panel2.innerHTML = '<div class="qrp-head"><span>PRZYPOMINAJKA</span><button type="button" class="qrp-close" aria-label="Zamknij">×</button></div><div class="qrp-body"></div>';
    ...
    async function claimCalendar() {
      ...
      const response = await ctx.game.request(
        `rewards_calendar&action=open&day_no=${dayNo}`,
        (packet) => Boolean(packet?.rewards_calendar) || Object.hasOwn(packet || {}, "e"),
        { strip: ["rewards_calendar"], signal: ctx.scheduler.signal }
      );
    ...
    async function claimPromotions() {
      ...
      for (const item of items) {
        if (!ctx.enabled) return false;
        const response = await ctx.game.request(
          `promotions&a=use&id=${encodeURIComponent(item.id)}`,
          (packet) => Object.hasOwn(packet || {}, "e"),
          { strip: ["promotions"], signal: ctx.scheduler.signal }
        );
    ...
    function destroyExpired() {
      ...
      const next = () => {
        ...
        if (typeof page2._g === "function") page2._g(`moveitem&st=-2&id=${encodeURIComponent(items[index].id)}`);
        index++;
        ctx.scheduler.timeout(next, 1e3);
      };
      next();
    }
    ...
    function render(force = false) {
      const body = panel2.querySelector(".qrp-body");
      const rows = [];
      ...
      panel2.dataset.claimable = String(claimable);
      panel2.querySelector(".qrp-close").hidden = claimable;
      panel2.hidden = !rows.length || !claimable && !force && signature === dismissed;
      showButton();
    }
    async function check(force = false) {
      if (checking || !ctx.enabled || page2.Engine?.allInit !== true) return;
      checking = true;
      ...
      try {
        ...
        if (ctx.settings.expiredEnabled) next.expired = expiredItems(page2);
        current = next;
        ...
        const text = unavailable ? count ? `Znaleziono: ${count}. Część danych jest niedostępna.` : "Nie udało się sprawdzić danych gry." : count ? `Znaleziono: ${count}` : "Brak oczekujących rzeczy.";
    ...
    function scheduleInterval() {
      ...
      intervalTimer = ctx.scheduler.timeout(() => {
        check();
        scheduleInterval();
      }, seconds * 1e3);
    }
    ctx.scheduler.listen(panel2.querySelector(".qrp-close"), "click", () => {
      if (current.calendar || current.promotions.length) return;
      dismissed = resultSignature(current);
      panel2.hidden = true;
    });
    ...
    ctx.scheduler.listen(panel2, "click", (event) => {
      ...
      if (!target) return;
      if (target.dataset.action !== "expired") {
      ...
      if (target.dataset.confirmed !== "true") {
        ...
        confirmationTimer = ctx.scheduler.timeout(() => {
          target.dataset.confirmed = "false";
          target.textContent = "Zniszcz bezpowrotnie";
        }, 5e3);
        return;
      }
  ...
  function createReminder() {
    return {
      ...
      renderSettings(ctx) {
        ...
        ctx.events.on("reminderStatus", (event) => {
  ...
  function compactPartyCss(settings) {
    ...
    const hpColumns = settings.hpPosition === "center" ? "minmax(0,1fr) auto minmax(0,1fr)" : "minmax(0,1fr) auto";
...
.party .party__list .party-member .qaddons-party-summary{position:absolute!important;left:${summaryLeft}px!important;right:var(--qaddons-party-actions-width,34px)!important;top:0!important;display:grid!important;grid-template-columns:${hpColumns}!important;align-items:center!important;height:${rowHeight}px!important;gap:5px!important;z-index:4!important;font-size:${fontSize}px!important;line-height:${rowHeight}px!important;pointer-events:none!important;white-space:nowrap!important}
...
`;
  ...
  var DEFAULTS5 = { hideAvatars: true, showHpPoints: false, hpPosition: "right", rowHeight: 18, fontSize: 9 };
  function value(node) {
  ...
  function createSummary() {
    ...
    left.className = "qaddons-party-left";
    ...
    left.append(nick, info);
    summary.append(left, hp);
    return summary;
  }
  function startCompactParty(ctx) {
    ...
    function syncMember(member) {
      ...
      const color2 = sourceNick ? getComputedStyle(sourceNick).color : "";
      if (color2 && nickNode.style.color !== color2) nickNode.style.color = color2;
      const title = [nick, info, percent, points].filter(Boolean).join(" ");
      if (summary.title !== title) summary.title = title;
      const visibleIcons = [...member.querySelectorAll(".info-icons > *")].filter((node) => getComputedStyle(node).display !== "none");
    ...
    return {
      ...
      renderSettings(ctx) {
        ...
        for (const input of section.querySelectorAll("[data-setting]")) ctx.scheduler.listen(input, input.type === "range" ? "input" : "change", () => {
          ...
          ctx.changeSettings({ [input.dataset.setting]: next });
          sync();
        });
        ctx.events.on("addonChanged", (event) => {
          if (event.id === ctx.id) sync();
  ...
  // src/addons/night-mode/style.js
  function number2(value2, fallback, minimum, maximum) {
    const parsed = Number(value2);
    return Number.isFinite(parsed) ? Math.min(maximum, Math.max(minimum, parsed)) : fallback;
  }
  function color(value2) {
    return /^#[0-9a-f]{6}$/i.test(String(value2 || "")) ? value2 : "#000000";
  }
  function normalizeNightSettings(settings = {}) {
    return {
      strength: number2(settings.strength, 45, 0, 85),
      color: color(settings.color),
      vignette: settings.vignette !== false
    };
  }
  function createMapBrightnessDrawable(page2, getSettings) {
    let active = true;
    return {
      get rx() {
        return Number(page2.Engine?.hero?.d?.x ?? page2.Engine?.hero?.x) || 0;
      },
      get ry() {
        return Number(page2.Engine?.hero?.d?.y ?? page2.Engine?.hero?.y) || 0;
      },
      disable() {
        active = false;
      },
      draw(context) {
        ...
        const size = page2.Engine?.getCanvasViewSize?.() || {};
  ...
  // src/addons/night-mode/index.js
  var DEFAULTS6 = { strength: 45, color: "#000000", vignette: true };
  function startNightMode(ctx) {
    const page2 = ctx.game.page;
    const drawable = createMapBrightnessDrawable(page2, () => ctx.settings);
    const addDrawable = () => page2.Engine?.renderer?.add?.(drawable);
    const api = page2.API;
    addDrawable();
  ...
  // src/addons/chat-autoscroll/runtime.js
  var CHAT_WRAPPER = ".new-chat-window .chat-message-wrapper, .chat-message-wrapper";
  function interval(value2) {
    const parsed = Number(value2);
    return Number.isFinite(parsed) ? Math.min(2e3, Math.max(100, Math.round(parsed))) : 250;
  }
  function startChatAutoscroll(ctx) {
    const page2 = ctx.game.page;
    let queued = 0;
    function forceBottom(update = false) {
      ...
      if (!ctx.enabled) return;
      try {
        const chat = page2.Engine?.chatController?.getChatMessageWrapper?.();
    ...
    ctx.scheduler.listen(document, "visibilitychange", () => {
      if (!document.hidden) forceBottom(true);
    });
    loop();
  }

  // src/addons/chat-autoscroll/index.js
  var DEFAULTS7 = { interval: 250 };
  function createChatAutoscroll() {
    return {
      id: "chat-autoscroll",
      name: "Czat zawsze na dole",
      ...
      renderSettings(ctx) {
        ...
        const sync = () => {
          enabled.checked = ctx.enabled;
          frequency.value = String([100, 250, 500, 1e3].includes(Number(ctx.settings.interval)) ? ctx.settings.interval : 250);
        };
        sync();
        ctx.scheduler.listen(enabled, "change", () => ctx.setEnabled(enabled.checked));
  ...
  // src/addons/auto-abyss/data.js
  var ABYSS_STATES = Object.freeze({
    NONE: 0,
    SEARCHING: 1,
    FOUND: 2,
    PREPARATION: 3,
  ...
  function normalized(value2) {
    return String(value2 || "").toLocaleLowerCase("pl-PL");
  }
  function normalizeBuilds(source) {
    if (!source || typeof source !== "object") return [];
    return Object.entries(source).map(([key, value2]) => {
  ...
  function inferBuild(builds, profession) {
    ...
    for (const build of list) {
      ...
      const compact = name.includes("exp") ? "" : name.replace(/pvp|1h|2h|ogień|ogien|błysk|blysk|zimno|fizyk|gr|truta/g, "").replace(/[^wpbmth]/g, "");
  ...
  function configuredBuild(builds, profession, setting, currentId = 0) {
  ...
  // src/addons/auto-abyss/runtime.js
  function engine(page2) {
    try {
  ...
  function readBuilds(page2) {
    try {
      const commons = engine(page2)?.buildsManager?.getBuildsCommons?.();
  ...
  function currentBuildId(page2) {
    try {
      return Number(engine(page2)?.buildsManager?.getBuildsCommons?.()?.getCurrentId?.()) || 0;
    } catch {
      return 0;
    }
  }
  function updateAbyssTracker(tracker, packet) {
    for (const data of packetList2(packet)) {
      if (data.matchmaking_state !== void 0) {
  ...
  function startAutoAbyss(ctx, tracker = {}) {
    const page2 = ctx.game.page;
    ...
    function send(command) {
      if (engine(page2)?.allInit !== true || typeof page2._g !== "function") throw new Error("Gra nie jest jeszcze gotowa.");
      page2._g(command);
    }
    ...
    async function selectBuildAndPrepare() {
      ...
      const builds = readBuilds(page2);
      const selected = configuredBuild(builds, profession, ctx.settings[key], currentBuildId(page2));
      if (selected) {
        report(`Zmiana zestawu na „${selected.name}” (${profession || "?"}).`);
        const skillshop = engine(page2)?.skills ? "&skillshop=1" : "";
        try {
    ...
    function act() {
      if (ctx.scheduler.disposed || engine(page2)?.allInit !== true || typeof page2._g !== "function") return;
      if (summaryPending) {
        schedule("nextmatch", () => {
      ...
      if (state === ABYSS_STATES.NONE && Date.now() >= holdQueueUntil) {
        schedule("signin", () => {
          send("match&a=signin");
          report("Zapisano do kolejki Otchłani.");
        });
      } else if (state === ABYSS_STATES.FOUND && Number(confirmation?.accept) === 0) {
        schedule("accept", () => {
          send("match&a=accept_opp&ans=1");
          report("Zaakceptowano przeciwnika.");
        });
      } else if (state === ABYSS_STATES.PREPARATION && preparation) {
        schedule("prepare", selectBuildAndPrepare, 550, 5e3);
      } else if (state === ABYSS_STATES.BATTLING && ctx.settings.autoFight) {
        schedule("autofight", () => {
          send("fight&a=f&enabled=1");
          report("Włączono automatyczną walkę.");
        }, 450, 1e4);
      }
    }
  ...
  // src/addons/auto-abyss/style.js
  var autoAbyssSettingsCss = `
...
`;
  ...
  // src/addons/auto-abyss/index.js
      ...
      renderSettings(ctx) {
        ...
        function renderBuilds() {
          ...
          for (const select of buildsRoot.querySelectorAll("select")) {
            const value2 = String(ctx.settings[select.dataset.key] || "auto");
            select.value = [...select.options].some((option) => option.value === value2) ? value2 : "auto";
        ...
        function showStatus(event) {
          ...
          status.querySelector("[data-progress]").textContent = event.progress ? `Etap ${event.progress.stage}/4 · ${event.progress.current}/${event.progress.max} pkt` : "Postęp pojawi się po zakończeniu walki.";
        }
        sync();
        renderBuilds();
  ...
  function stripClanMembers(packet) {
    let stripped = false;
    const visit = (value2) => {
      ...
      if (Object.hasOwn(value2, "members")) {
        delete value2.members;
        stripped = true;
      }
    };
    visit(packet);
...
`;
  ...
  function startClanOnline(ctx) {
    ...
    function placePanel() {
      ...
      panel2.style.left = `${Math.max(0, Math.min(window.innerWidth - width, x || 0))}px`;
      panel2.style.top = `${Math.max(0, Math.min(window.innerHeight - height, Number(ctx.settings.windowY) || 70))}px`;
    }
    function persistGeometry() {
      if (panel2.hidden || !panel2.isConnected) return;
      const next = {
        windowX: Math.round(panel2.offsetLeft),
        windowY: Math.round(panel2.offsetTop),
        windowWidth: Math.round(panel2.offsetWidth),
        windowHeight: Math.round(panel2.offsetHeight)
      };
      if (Object.entries(next).some(([key, value2]) => Number(ctx.settings[key]) !== value2)) ctx.changeSettings(next);
    }
    ...
    async function refresh(force = false) {
      if (busy || document.hidden || page2.Engine?.allInit !== true || typeof page2._g !== "function") return;
    ...
    if (typeof ResizeObserver === "function") ctx.scheduler.observer(ResizeObserver, scheduleGeometrySave).observe(panel2);
    ...
    ctx.events.on("visibilityChanged", (hidden) => {
      if (!hidden && !panel2.hidden) refresh();
    });
  ...
  // src/addons/pocket-berserk/data.js
  var SOLO_BERSERK_ID = 34;
  var GROUP_BERSERK_ID = 35;
  var OFFSET_MIN = -50;
  var OFFSET_MAX = 13;
  var DEFAULTS10 = Object.freeze({
    showButton: true
  });
  var DEFAULT_MODE = Object.freeze({
    v: false,
    common: true,
    elite: true,
    elite2: true,
  ...
  function unwrap(value2, fallback) {
    const raw = value2 && typeof value2 === "object" && Object.hasOwn(value2, "v") ? value2.v : value2;
  ...
  function normalizeMode(raw, previous = DEFAULT_MODE) {
    ...
    for (const key of ["lvlmin", "lvlmax"]) {
      ...
      const value2 = Number(unwrap(raw[key], mode[key]));
      if (Number.isFinite(value2)) mode[key] = Math.max(OFFSET_MIN, Math.min(OFFSET_MAX, Math.round(value2)));
    }
    return mode;
  }
  function settingEntries(settings) {
    ...
    const list = Array.isArray(settings.list) ? settings.list : settings.list && typeof settings.list === "object" ? Object.entries(settings.list).map(([id, value2]) => ({ id, d: value2?.d || value2 })) : [];
    for (const entry of list) {
      ...
      for (const [key, value2] of Object.entries(entry)) {
        if ([SOLO_BERSERK_ID, GROUP_BERSERK_ID].includes(Number(key))) entries.push([Number(key), value2?.d || value2]);
      }
    }
    return entries;
  }
  function packetList4(packet) {
    return Array.isArray(packet) ? packet.flat(Infinity).filter(Boolean) : [packet].filter(Boolean);
  }
  function partyPresent(party) {
    if (!party || typeof party !== "object") return false;
    const members = party.members && typeof party.members === "object" ? party.members : party;
    return Object.entries(members).some(([key, value2]) => /^\d+$/.test(key) || Number.isFinite(Number(value2?.id)));
  }
  function createBerserkTracker() {
    return { modes: { [SOLO_BERSERK_ID]: null, [GROUP_BERSERK_ID]: null }, inParty: false, heroLevel: 0 };
  }
  function updateBerserkTracker(tracker, packet) {
    let changed = false;
    for (const data of packetList4(packet)) {
      if (!data || typeof data !== "object") continue;
      const hero = data.h || data.hero;
      if (hero && typeof hero === "object") {
        const level = Number(hero.oplvl ?? hero.d?.oplvl ?? hero.lvl ?? hero.d?.lvl);
        if (Number.isFinite(level) && level !== tracker.heroLevel) {
          tracker.heroLevel = level;
          changed = true;
        }
      }
      if (Object.hasOwn(data, "party")) {
        const inParty = partyPresent(data.party);
        if (inParty !== tracker.inParty) {
          tracker.inParty = inParty;
          changed = true;
        }
      }
      for (const [id, raw] of settingEntries(data.settings)) {
        const next = normalizeMode(raw, tracker.modes[id] || DEFAULT_MODE);
        if (JSON.stringify(next) !== JSON.stringify(tracker.modes[id])) {
          tracker.modes[id] = next;
          changed = true;
        }
      }
    }
  ...
  function settingCommand(id, key, value2) {
    const suffix = key ? `&key=${encodeURIComponent(key)}` : "";
    return `settings&action=update&id=${Number(id)}${suffix}&v=${encodeURIComponent(Number(Boolean(value2)))}`;
  }
  function levelCommand(id, key, value2) {
    const offset = Math.max(OFFSET_MIN, Math.min(OFFSET_MAX, Math.round(Number(value2) || 0)));
    return `settings&action=update&id=${Number(id)}&key=${encodeURIComponent(key)}&v=${offset}`;
  }

  // src/addons/pocket-berserk/style.js
  var POCKET_BERSERK_CSS = `
...
`;
  ...
  // src/addons/pocket-berserk/runtime.js
  function heroOperationalLevel(page2, tracker) {
    const hero = page2.Engine?.hero?.d || page2.Engine?.hero || page2.g?.hero || {};
    return Number(hero.oplvl ?? hero.lvl ?? tracker.heroLevel) || 0;
  }
  function sendBerserkSetting(page2, tracker, id, key, value2) {
    if (page2.Engine?.allInit !== true || typeof page2._g !== "function") return false;
    ...
    if (key === "lvlmin" || key === "lvlmax") {
      ...
      page2._g(levelCommand(id, key, offset));
    } else {
      page2._g(settingCommand(id, key, value2));
    }
    return true;
  }
  function startPocketBerserk(ctx, tracker) {
    const page2 = ctx.game.page;
    ...
    function currentId() {
      return tracker.inParty ? GROUP_BERSERK_ID : SOLO_BERSERK_ID;
    }
    function render() {
      const id = currentId();
      const mode = tracker.modes[id];
...
Klik: przełącz · PPM: ustawienia` : "Kieszonkowy berserk: oczekiwanie na ustawienia gry";
    ...
    ctx.scheduler.listen(button, "click", () => {
      ...
      const mode = tracker.modes[id];
      if (!mode) return;
      sendBerserkSetting(page2, tracker, id, "", !mode.v);
      render();
  ...
  // src/addons/pocket-berserk/index.js
  var MODE_NAMES = Object.freeze({ [SOLO_BERSERK_ID]: "Samotnie", [GROUP_BERSERK_ID]: "W drużynie" });
  function createPocketBerserk() {
    ...
    return {
      ...
      renderSettings(ctx) {
        ...
        for (const box of section.querySelectorAll("[data-mode]")) for (const input of box.querySelectorAll("[data-key]")) ctx.scheduler.listen(input, "change", () => {
          ...
          const key = input.dataset.key;
          const level = heroOperationalLevel(ctx.game.page, tracker);
          const value2 = input.type === "checkbox" ? input.checked : Number(input.value) - level;
          sendBerserkSetting(ctx.game.page, tracker, id, key, value2);
          ctx.events.emit("pocketBerserkChanged");
          sync();
        });
...
`;
  ...
  var DEFAULTS11 = Object.freeze({ locked: false, showHandle: true, x: null, y: null });
    ...
    return {
      ...
      renderSettings(ctx) {
        ...
        function sync() {
          ...
          showHandle.disabled = !ctx.enabled;
  ...
  var PARTY_SUMMON_ACCEPT_COMMAND = "party&a=acceptsummon&answer=1";
  var PARTY_SUMMON_PHRASES = Object.freeze([
    "przyzywa do siebie swoją drużynę",
    "is summoning your party"
  ]);
  function partySummonPrompt(value2) {
    const message = typeof value2 === "string" ? value2 : value2?.q;
    if (typeof message !== "string") return false;
    const normalized2 = message.toLocaleLowerCase("pl-PL");
    return PARTY_SUMMON_PHRASES.some((phrase) => normalized2.includes(phrase));
  }
  function normalizeOthers(source) {
    const entries = source instanceof Map ? [...source.entries()] : Array.isArray(source) ? source.map((value2, index) => [index, value2]) : source && typeof source === "object" ? Object.entries(source) : [];
...
`;
  ...
  function currentOthers(page2) {
    ...
    try {
      const sources = [model?.check?.(), model?.getAll?.(), model?.others, model?.list, page2.g?.other];
      const found = /* @__PURE__ */ new Map();
      for (const source of sources) for (const other of normalizeOthers(source)) found.set(other.id, other);
      return [...found.values()];
    } catch {
      return normalizeOthers(page2.g?.other);
    }
  }
  ...
  function partyState(page2, tracked) {
    ...
    try {
      ...
      if (current instanceof Map) {
        const ids2 = new Set([...current.entries()].map(([key, member]) => String(member?.id ?? key)));
        const leader = [...current.entries()].find(([, member]) => member?.leader || member?.commander);
        return { exists: party.isPartyPlayers?.() ?? ids2.size > 0, ids: ids2, commanderId: leader ? String(leader[1]?.id ?? leader[0]) : null };
      }
    } catch {
    }
  ...
  function startQuickGroup(ctx) {
    ...
    function invite() {
      ...
      const others = currentOthers(page2);
      ...
      showMessage(page2, candidates.length ? `Wysłano ${candidates.length} zaproszeń do grupy.` : others.length ? `Na mapie jest ${others.length} postaci, ale żadna nie spełnia ustawionych warunków zapraszania.` : "Nie udało się odczytać żadnej postaci z aktualnej mapy.", !candidates.length);
    ...
    function hookPartySummon() {
      if (alertWrapper || typeof page2.mAlert !== "function") return;
      originalAlert = page2.mAlert;
      alertWrapper = function(...args) {
        if (ctx.settings.autoAcceptSummon && partySummonPrompt(args[0]) && typeof page2._g === "function") {
          page2._g(PARTY_SUMMON_ACCEPT_COMMAND);
          showMessage(page2, "Automatycznie zaakceptowano przywołanie drużyny.");
          return false;
        }
        return originalAlert.apply(this, args);
      };
      page2.mAlert = alertWrapper;
    }
    hookPartySummon();
    ...
    ctx.scheduler.cleanup(() => {
      if (page2.mAlert === alertWrapper) page2.mAlert = originalAlert;
      button.remove();
    });
  }
  ...
  // src/addons/teleport-labels/data.js
  var DEFAULTS13 = Object.freeze({
    labels: true,
    customLabels: {},
    fontSize: 9,
    color: "#ffffff",
  ...
  var SUMMON_LABELS = Object.freeze({
    "Domina Ecclesiae": "DOMI",
    "Mietek Żul": "ŻUL",
    "Mroczny Patryk": "PAT",
    "Karmazynowy Mściciel": "KARM",
    "Złodziej": "ZŁOD",
    "Zły Przewodnik": "ZŁY",
    "Opętany Paladyn": "OPEK",
    "Piekielny Kościej": "KOST",
    "Koziec Mściciel Ścieżek": "KOZIEC",
    "Kochanka Nocy": "KOCH",
    "Książę Kasim": "KASIM",
    "Święty Braciszek": "BRAT",
    "Złoty Roger": "ROG",
    "Baca bez Łowiec": "BACA",
    "Czarująca Atalia": "ATKA",
    "Obłąkany Łowca Orków": "OBŁO",
    "Lichwiarz Grauhaz": "LICH",
    "Viviana Nandin": "VIVA",
    "Przeraza": "PRZE",
    "Demonis Pan Nicości": "DEM",
    "Mulher Ma": "MUHA",
    "Vapor Veneno": "VAPO",
    "Dęborożec": "DĘBO",
    "Tepeyollotl": "TEP",
    "Widmo Triady": "TRIO",
    "Negthotep Czarny Kapłan": "NEGH",
    "Młody Smok": "SMOK"
  });
  var SUMMON_PREFIX = "Zwój przywołania drużyny na herosa ";
  function parseItemStats(item) {
    const value2 = typeof item?.stat?.stat === "string" ? item.stat.stat : item?.stat;
    if (typeof value2 !== "string") return item?._cachedStats || value2 || {};
    return Object.fromEntries(value2.split(";").filter(Boolean).map((part) => {
      const separator = part.indexOf("=");
  ...
  function sanitizeLabel(value2) {
    return [...String(value2 || "").trim().replace(/\s+/g, " ").toLocaleUpperCase("pl-PL")].slice(0, 8).join("").trim();
  }
  function initials(value2) {
    const words = String(value2 || "").match(/\p{L}+/gu) || [];
    return sanitizeLabel(words.length > 1 ? words.map((word) => [...word][0]).join("") : words[0] || "TP");
  }
  function plainText(value2) {
    return String(value2 || "").replace(/<\s*(?:br|\/div|\/p)\s*\/?>/gi, "\n").replace(/<[^>]*>/g, " ").replaceAll("&nbsp;", " ").replaceAll("&oacute;", "ó").replaceAll("&Oacute;", "Ó").replaceAll("&amp;", "&").replaceAll("&lt;", "<").replaceAll("&gt;", ">").replace(/[ \t]+/g, " ");
  }
  function destinationLabel(value2) {
    const words = String(value2 || "").trim().replace(/[.,:;!?]+$/, "").match(/\p{L}+/gu) || [];
  ...
  function labelForItem(item, customLabels = {}) {
  ...
  function parseCustomLabels(value2) {
    const result = {};
    for (const line of String(value2 || "").split(/\r?\n/)) {
  ...
  function serializeCustomLabels(value2) {
    return Object.entries(value2 || {}).map(([key, label]) => `${key}=${sanitizeLabel(label)}`).join("\n");
  }

  // src/addons/teleport-labels/style.js
  function teleportLabelsCss(settings) {
    const size = Math.max(7, Math.min(14, Math.round(Number(settings.fontSize) || 9)));
    const color2 = /^#[0-9a-f]{6}$/i.test(settings.color || "") ? settings.color : "#ffffff";
    ...
    const shadow = settings.shadow === "none" ? "none" : settings.shadow === "soft" ? `0 1px ${Math.max(1, strength)}px ${shadowColor}` : settings.shadow === "glow" ? `0 0 ${Math.max(1, strength)}px ${shadowColor},0 0 ${Math.max(2, strength * 2)}px ${shadowColor}` : `-1px -1px 0 ${shadowColor},1px -1px 0 ${shadowColor},-1px 1px 0 ${shadowColor},1px 1px 0 ${shadowColor}`;
    return `
.qaddons-teleport-label{position:absolute!important;top:0!important;bottom:auto!important;left:50%;z-index:24;max-width:64px;transform:translateX(-50%);overflow:visible;pointer-events:none!important;color:${color2}!important;background:transparent!important;border:0!important;padding:0!important;font:${settings.bold === false ? "normal" : "bold"} ${size}px/${size + 1}px Arial,sans-serif!important;text-align:center!important;white-space:nowrap;text-shadow:${shadow}!important;}
`;
  }

  // src/addons/teleport-labels/runtime.js
  function values2(value2) {
    if (value2 instanceof Map) return [...value2.values()];
    if (Array.isArray(value2)) return value2;
    return value2 && typeof value2 === "object" ? Object.values(value2) : [];
  }
  function itemId2(item, fallback = "") {
    const id = item?.id ?? fallback;
    return /^-?\d+$/.test(String(id)) ? String(id) : "";
  }
  function engineItems(page2) {
    const result = [];
    const model = page2.Engine?.items;
    for (const location of ["g", "e", "p"]) {
      try {
        result.push(...values2(model?.fetchLocationItems?.(location)));
      } catch {
      }
    }
    try {
      result.push(...values2(model?.testMyItems?.()));
    } catch {
    }
    result.push(...values2(model?.items));
    return result.filter((item) => item && typeof item === "object");
  }
  function startTeleportLabels(ctx) {
    const page2 = ctx.game.page;
    ...
    function remember(items, replace = false) {
      for (const [fallback, item] of Object.entries(items || {})) {
        const id = itemId2(item, fallback);
        if (!id) continue;
    ...
    function sync() {
      frame = 0;
      remember(Object.fromEntries(engineItems(page2).map((item, index) => [itemId2(item, index), item])), true);
    ...
    const poll = () => {
      schedule();
      ctx.scheduler.timeout(poll, 750);
    };
  ...
  // src/addons/teleport-labels/index.js
  function createTeleportLabels() {
    return {
      id: "teleport-labels",
      name: "Podpisownik",
      ...
      renderSettings(ctx) {
        const section = document.createElement("section");
        section.className = "mtk-addon-settings qtl-settings";
...
name:Domina Ecclesiae=DOMI"></textarea></label>
        ...
        ctx.scheduler.listen(section.querySelector("[data-save]"), "click", () => {
          const labels = parseCustomLabels(custom.value);
          ctx.changeSettings({ customLabels: labels });
          custom.value = serializeCustomLabels(labels);
          status.textContent = `Zapisano ${Object.keys(labels).length} własnych podpisów.`;
        });
        ctx.scheduler.listen(section.querySelector("[data-clear]"), "click", () => {
          custom.value = "";
          ctx.changeSettings({ customLabels: {} });
          status.textContent = "Własne podpisy zostały wyczyszczone.";
        });
        ctx.events.on("addonChanged", (event) => {
          if (event.id === ctx.id) sync();
        });
...
`;
  ...
  function clampGeometry(settings, viewportWidth, viewportHeight) {
...
#qaddons-build-switcher{position:fixed;left:0;top:0;z-index:32020;box-sizing:border-box;width:300px;height:150px;min-width:220px;min-height:90px;max-width:100vw;max-height:100vh;overflow:auto;resize:both;border:1px solid #777;background:#000;color:#ddd;box-shadow:0 5px 20px #000;font:11px Arial}
#qaddons-build-switcher[hidden]{display:none}#qaddons-build-switcher header{display:flex;align-items:center;justify-content:space-between;height:27px;padding:0 7px;border-bottom:1px solid #555;font-weight:700;cursor:move;user-select:none;touch-action:none}
...
`;
  ...
  function startBuildSwitcher(ctx) {
    ...
    function applyGeometry() {
      ...
      panel2.style.left = `${geometry.x}px`;
      panel2.style.top = `${geometry.y}px`;
      panel2.style.width = `${geometry.width}px`;
      panel2.style.height = `${geometry.height}px`;
      ctx.scheduler.frame(() => {
        applyingGeometry = false;
      });
    }
    function persistGeometry() {
      if (panel2.hidden || !panel2.isConnected || applyingGeometry) return;
      const geometry = clampGeometry({
        windowX: panel2.offsetLeft,
        windowY: panel2.offsetTop,
        windowWidth: panel2.offsetWidth,
        windowHeight: panel2.offsetHeight
      }, innerWidth, innerHeight);
      panel2.style.left = `${geometry.x}px`;
      panel2.style.top = `${geometry.y}px`;
      const patch = { windowX: geometry.x, windowY: geometry.y, windowWidth: geometry.width, windowHeight: geometry.height };
      if (Object.entries(patch).some(([key, value2]) => Number(ctx.settings[key]) !== value2)) ctx.changeSettings(patch);
    }
    function scheduleGeometrySave() {
...
PPM — więcej opcji`;
    ...
    const header = panel2.querySelector("header");
    ...
    ctx.scheduler.listen(header, "pointermove", (event) => {
      ...
      const geometry = clampGeometry({
        windowX: event.clientX - drag.offsetX,
        windowY: event.clientY - drag.offsetY,
        windowWidth: panel2.offsetWidth,
        windowHeight: panel2.offsetHeight
      }, innerWidth, innerHeight);
      panel2.style.left = `${geometry.x}px`;
    ...
    if (typeof ResizeObserver === "function") ctx.scheduler.observer(ResizeObserver, scheduleGeometrySave).observe(panel2);
    ctx.scheduler.listen(window, "resize", () => {
      persistGeometry();
      applyGeometry();
    }, { passive: true });
    ctx.events.on("gamePacket", (packet) => {
    ...
    return {
      ...
      renderSettings(ctx) {
        ...
        section.innerHTML = `<h2>Zmieniacz zestawów</h2><label class="mtk-enabled"><input type="checkbox" data-enabled> Dodatek aktywny</label>
                ...
                <label class="ln-field">Liczba kolumn<select data-setting="columns">${[1, 2, 3, 4, 5].map((value2) => `<option value="${value2}">${value2}</option>`).join("")}</select></label>
