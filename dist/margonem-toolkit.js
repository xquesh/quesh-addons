...
`);
  ...
  function characterDestination(hero, page2) {
    let domain = typeof page2.getMainDomain === "function" ? page2.getMainDomain() : "";
    if (!/^(pl|com)$/.test(domain)) domain = page2.location.hostname.match(/(?:^|\.)margonem\.(pl|com)$/)?.[1];
    if (!domain) {
      try {
  ...
  function reloadCharacter(hero, page2, navigate = (url) => page2.location.replace(url)) {
    ...
    page2.setCookie("mchar_id", target.id, new Date(Date.now() + 2592e6), "/", target.cookieDomain, true);
    navigate(target.url);
    return "reload";
  }
