...
`);
  ...
  var defaults4 = { sort: "level-desc", horizontal: 100, showTimers: true, hotkeys: false, selectedWorld: "", collapsed: false };
  ...
  var barStyle = `
#qaddons-relogger{position:fixed;z-index:2147482990;display:flex;align-items:center;gap:3px;width:max-content;max-width:calc(100vw - 8px);height:38px;color:#ddd;background:transparent;border:0;box-shadow:none;font:10px Arial,sans-serif;}
...
#qaddons-relogger select{width:38px;height:20px;}
#qaddons-relogger select option{font-size:12px;}
#qaddons-relogger [data-collapse]{width:13px;height:20px;}
#qaddons-relogger .qr-body{min-width:0;}
#qaddons-relogger .qr-cards{display:flex;gap:2px;max-width:250px;overflow-x:auto;scrollbar-width:none;}
#qaddons-relogger .qr-cards::-webkit-scrollbar{display:none;}
#qaddons-relogger .qr-card{flex:0 0 26px;width:26px;height:38px;position:relative;overflow:hidden;border:1px solid #343a3e;border-bottom:2px solid #454545;}
#qaddons-relogger .qr-portrait{position:absolute;left:2px;top:0;width:32px;height:48px;transform:scale(.65);transform-origin:top left;background-position:0 0;background-repeat:no-repeat;text-align:center;line-height:44px;font-size:26px;color:#aaa;}
#qaddons-relogger .qr-nick,#qaddons-relogger .qr-level{display:none;}
#qaddons-relogger .qr-time{position:absolute;bottom:0;left:0;right:0;font:bold 7px/9px Arial;white-space:nowrap;text-align:center;text-shadow:0 1px 2px #000,-1px 0 #000,1px 0 #000;background:#0009;}
...
`;
  ...
  function startRelogger(ctx) {
    ...
    function position() {
      ...
      const bottom = anchorRect?.height > 0 ? anchorRect.bottom : page2.innerHeight;
      bar.style.left = `${Math.max(0, left + 4 + Math.max(0, right - left - width - 8) * horizontal / 100)}px`;
      bar.style.top = `${Math.max(0, Math.min(page2.innerHeight - 38, bottom - 40))}px`;
    ...
    return {
      ...
      renderSettings(ctx) {
        ...
        ctx.scheduler.listen(section.querySelector("[data-reset]"), "click", () => {
          ctx.changeSettings({ horizontal: 100, collapsed: false });
