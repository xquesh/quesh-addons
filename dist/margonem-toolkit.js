  ...
  function createGame(page2, events2, scheduler2, host = window) {
    ...
    function request(command, match, options = {}) {
      return new Promise((resolve, reject) => {
        if (stopped || typeof page2._g !== "function") return reject(new Error("Gra nie jest jeszcze gotowa."));
    ...
    return { page: page2, start: start2, destroy, request, get hooked() {
...
`);
  ...
  function createTooltipTools(tooltipConfig) {
    ...
    function itemById(itemId2) {
      try {
        if (!itemId2) return null;
        const items = window.Engine?.items;
        if (typeof items?.getItemById === "function") {
          const direct = items.getItemById(itemId2);
          if (direct) return direct;
        }
        return items?.items?.[itemId2] || null;
      } catch {
        return null;
      }
    }
    ...
    function lootHtml(itemId2, info, fullHtml) {
      const hid = currentHid(fullHtml);
      let item = itemById(itemId2);
  ...
  var defaults4 = { sort: "level-desc", barPosition: "bottom", horizontal: 100, showWorldButton: true, showTimers: true, hotkeys: false, selectedWorld: "", switchMode: "direct" };
  ...
  function characterDestination(hero, page2) {
    let domain = page2.location.hostname.match(/(?:^|\.)margonem\.(pl|com)$/)?.[1];
    if (!domain) {
      try {
        domain = new URL(page2.Engine?.worldConfig?.getApiDomain?.()).hostname.match(/(?:^|\.)margonem\.(pl|com)$/)?.[1];
      } catch {
      }
    }
  ...
  function relogTarget(hero, page2) {
    const engine2 = page2.Engine;
    if (engine2?.allInit !== true) throw new Error("Poczekaj na załadowanie gry.");
    if (engine2.changePlayer?.id != null) throw new Error("Zmiana postaci już trwa.");
    if (engine2.dialogue && typeof engine2.dialogue === "object") throw new Error("Najpierw zamknij rozmowę z NPC.");
    if (String(engine2.hero?.d?.id || page2.getCookie?.("mchar_id")) === String(hero.id)) throw new Error("Ta postać jest już zalogowana.");
    return characterDestination(hero, page2);
  }
  function reloadCharacter(hero, page2, navigate = (url) => page2.location.replace(url), schedule = (callback, delay) => page2.setTimeout(callback, delay)) {
    const target = characterDestination(hero, page2);
    if (typeof page2.setCookie !== "function") throw new Error("Gra nie udostępnia zmiany postaci.");
    const canCloseSession = typeof page2._g === "function" && typeof page2.setTimeout === "function";
    if (canCloseSession) page2._g("logoff&a=start");
  ...
  function changeCharacter(hero, page2, navigate = (url) => page2.location.replace(url)) {
    relogTarget(hero, page2);
    return reloadCharacter(hero, page2, navigate);
  }
  function changeCharacterNative(hero, page2) {
    const target = relogTarget(hero, page2);
    if (page2.Engine?.logOff) throw new Error("Trwa już wylogowywanie. Anuluj je przed zmianą postaci.");
    if (typeof page2.Engine?.changePlayer?.changePlayerRequest === "function") {
      page2.Engine.changePlayer.changePlayerRequest(Number(target.id));
      return "native";
    }
    if (typeof page2.Engine?.changePlayer?.changePlayer === "function") {
      page2.Engine.changePlayer.changePlayer(Number(target.id));
      return "native";
    }
    return reloadCharacter(hero, page2);
  }
...
${timers.length ? timers.map((timer) => `${timer.name}: ${timer.text}${timer.state === "window" ? " · możliwy respawn" : ""}`).join("\n") : settings.showTimers && !available ? "Dane timerów niedostępne" : "Brak aktywnych timerów"}`;
    ...
    function unlockRelog(message = "") {
      scheduler2.clearTimeout(relogTimeout);
      scheduler2.clearTimeout(recoveryTimeout);
      relogTimeout = 0;
      recoveryTimeout = 0;
      relogging = false;
      reloggingFrom = "";
      reloggingObserved = false;
      relogStartedAt = 0;
      relogDeadline = 0;
      relogTargetHero = null;
    ...
    function relog(hero) {
      ...
      try {
        reloggingFrom = String(page2.Engine?.hero?.d?.id || "");
        relogTargetHero = hero;
        if (settings.switchMode === "native") changeCharacterNative(hero, page2);
        else changeCharacter(hero, page2);
        relogging = true;
        error = "";
        reloggingObserved = page2.Engine?.changePlayer?.id != null;
        relogStartedAt = Date.now();
        relogDeadline = settings.switchMode === "native" ? relogStartedAt + 7e3 : 0;
        entries.forEach((button) => {
          button.disabled = true;
        });
        updateTimers();
        relogTimeout = scheduler2.timeout(() => unlockRelog("Zmiana postaci nie zakończyła się. Możesz spróbować ponownie."), 14e3);
      } catch (cause) {
        reloggingFrom = "";
        reloggingObserved = false;
        relogStartedAt = 0;
    ...
    function finishStalledRelog() {
      ...
      updateTimers();
      try {
        const changer = page2.Engine?.changePlayer;
        if (changer) changer.id = Number(relogTargetHero.id);
        if (page2.Engine?.logOff && typeof page2.Engine.logOff.out === "function") {
          page2.Engine.stop?.();
          page2.Engine.logOff.out();
        } else if (typeof changer?.reloadPlayer === "function") changer.reloadPlayer(Number(relogTargetHero.id));
        else reloadCharacter(relogTargetHero, page2);
        recoveryTimeout = scheduler2.timeout(() => {
          const active = String(page2.Engine?.hero?.d?.id || "");
          if (relogging && active !== String(relogTargetHero?.id || "")) {
            try {
              reloadCharacter(relogTargetHero, page2);
            } catch (cause) {
              unlockRelog(cause.message);
            }
          }
        }, 900);
      } catch (cause) {
        unlockRelog(cause.message || "Nie udało się dokończyć zmiany postaci.");
      }
    }
    ...
    ctx.events.on("gamePacket", (packet) => {
      ...
      for (const data of packets) {
        ...
        if (seconds > 0) {
          ...
          relogDeadline = Math.max(relogDeadline, relogStartedAt + 7e3, Date.now() + seconds * 1e3 + 1500);
        } else if (seconds === 0) {
          unlockRelog();
          render();
        }
    ...
    const tick = () => {
      const user = page2.getCookie?.("user_id") || "";
      const activeHero = String(page2.Engine?.hero?.d?.id || "");
      const changePending = page2.Engine?.changePlayer?.id != null;
      if (relogging && changePending) reloggingObserved = true;
      if (relogging && activeHero && activeHero !== reloggingFrom) {
        unlockRelog();
        render();
    ...
    return {
      ...
      renderSettings(ctx) {
        ...
        section.innerHTML = `<h2>Przelogawka</h2><label class="mtk-enabled"><input type="checkbox" data-enabled> Dodatek aktywny</label>
                ...
                </div><p>Tryb szybki zgłasza serwerowi zamknięcie bieżącej sesji, zapisuje wybraną postać i po 0,5 s ładuje właściwy świat bez pięciosekundowego okna. Dzięki temu nowa sesja otrzymuje pełne dane postaci i ekwipunku. Tryb natywny korzysta z okna gry.</p><p>Skróty wybierają postacie w kolejności na belce, na wybranym świecie. Nie działają podczas wpisywania tekstu w polach formularzy i czacie.</p>
...
.qe-slot{position:relative;flex:1 1 96px;min-width:96px;min-height:58px;padding:5px;border:1px dashed #666;background:#090909;color:#bbb;text-align:left;overflow:hidden}
...
.qe-slot[data-filled="true"]{padding-left:43px}.qe-item-preview{position:absolute;left:5px;top:50%;width:32px;height:32px;transform:translateY(-50%);pointer-events:none}.qe-item-preview>.qe-item-image{position:relative!important;inset:auto!important;display:block!important;width:32px!important;height:32px!important;margin:0!important;opacity:1!important;visibility:visible!important;transform:none!important}
...
`;
  ...
  function startEnhancer(ctx) {
    ...
    ctx.scheduler.listen(toggle, "click", () => {
    ...
    ctx.scheduler.listen(windowElement.querySelector("[data-close]"), "click", () => {
      windowElement.hidden = true;
    });
...
`;
  ...
  function color(value2) {
    return /^#[0-9a-f]{6}$/i.test(String(value2 || "")) ? value2 : "#07101c";
  }
  function nightModeCss(settings) {
    const opacity = number2(settings.strength, 45, 0, 85) / 100;
    const nightColor = color(settings.color);
    const background = settings.vignette === false ? nightColor : `radial-gradient(circle at center,transparent 30%,#000 125%),${nightColor}`;
    return `
...
`;
  ...
  var DEFAULTS6 = { strength: 45, color: "#07101c", vignette: true };
  function startNightMode(ctx) {
    ...
    ctx.events.on("nightModeChanged", apply2);
  ...
  function startAutoAbyss(ctx, tracker = {}) {
    ...
    function send(command) {
      if (typeof page2._g !== "function") throw new Error("Gra nie jest jeszcze gotowa.");
      page2._g(command);
    }
    ...
    function act() {
      if (ctx.scheduler.disposed || typeof page2._g !== "function") return;
      if (summaryPending) {
        schedule("nextmatch", () => {
...
`;
  ...
  function sendBerserkSetting(page2, tracker, id, key, value2) {
    if (typeof page2._g !== "function") return false;
