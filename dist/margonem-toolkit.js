(() => {
  ...
  function createTestLoot(state, ctx, runtime) {
    ...
    function testLoot() {
      ...
      const heroId2 = ctx.game.page.Engine?.hero?.d?.id ?? ctx.game.page.Engine?.hero?.id ?? ctx.game.page.g?.hero?.id ?? 0;
...
`);
  ...
  function heroTimers(data, heroId2, now, fadeout = 600) {
    if (!Array.isArray(data)) return [];
    const expiry = Number.isFinite(Number(fadeout)) && Number(fadeout) >= 0 ? Number(fadeout) : 600;
    return data.filter((timer) => timer && String(timer.heroData?.id) === String(heroId2) && (timer.type === 2 || timer.type === "hero" || timer.user === 1) && Number.isFinite(Number(timer.presp)) && Number(timer.presp) > 0 && now - Number(timer.presp) < expiry).map((timer) => {
