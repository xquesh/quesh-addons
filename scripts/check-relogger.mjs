...
assert.deepEqual(nativeCalls, [['changePlayer', 2]]);
const requestCalls = [];
...
assert.equal(changeCharacter(list[1], requestPage), 'native');
assert.deepEqual(requestCalls, [2]);
requestPage.Engine.logOff = true;
assert.throws(() => changeCharacter(list[1], requestPage), /Trwa już wylogowywanie/);
