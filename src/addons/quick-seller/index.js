...
export function createQuickSeller() {
    return {
        ...
        renderSettings(ctx) {
            ...
            const sync = () => { enabled.checked = ctx.enabled; hotkey.value = ctx.settings.hotkey || 'Brak'; interval.value = ctx.settings.interval; section.querySelector('[data-interval]').textContent = `${ctx.settings.interval} ms`; };
            sync();
